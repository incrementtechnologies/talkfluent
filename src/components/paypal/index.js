...
export class PayPalPermission{
  ...
  _sendRequest (action, requestData, callback) {
    ...
    const req = client.post(this._buildRequestUrl(action), requestData, function (data, response) {
      ...
      if (data.responseEnvelope.ack === 'Failure' || data.responseEnvelope.ack === 'FailureWithWarning') {
        err = data.error[0]
      }
      callback(err, data)
    })
  ...
  _sendRequestByCustom (action, data) {
    let request = jQuery.ajax({
      url: this._buildRequestUrl(action),
      data: data,
      type: 'POST',
  ...
  requestPermissions (scope, returnUrl, callback) {
    ...
    this._sendRequestByCustom('RequestPermissions', data)
  }
