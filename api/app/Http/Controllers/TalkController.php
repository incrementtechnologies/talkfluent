...
class TalkController extends Controller
{
  ...
  */
  ...
  public function checkEnabledLessons($accountId){
    $result = EnabledLesson::where('account_id', '=', $accountId)->get();

    return (sizeof($result) > 0) ? true : false;
  }

  public function updateBilling($accountId){
    $billing = Billing::where('account_id', '=', $accountId)->where('status', '=', 'created')->orderBy('created_at', 'DESC')->first();

    if($billing){
      $pStartDate = Carbon::createFromFormat('Y-m-d H:i:s', $billing->start_date);
      $currentStart = Carbon::now();
      $diffStart = $currentStart->diffInSeconds($pStartDate);
      if($diffStart > 0){
